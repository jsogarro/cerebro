...
class TestReportModels:
    ...
    def test_create_report_request_minimal(self):
        ...
        request = CreateReportRequest(title="Minimal Report", query="Simple query")
    ...
    def test_report_response_from_db_report(self):
        ...
        assert len(response.download_urls) == 2
        assert (
            response.download_urls["html"]
            == f"http://localhost:8000/reports/{report_id}/download/html"
        )
        assert (
            response.download_urls["pdf"]
            == f"http://localhost:8000/reports/{report_id}/download/pdf"
        )
