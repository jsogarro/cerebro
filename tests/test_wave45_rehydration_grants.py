...
@pytest.mark.asyncio
async def test_rehydrate_restores_contract_identity_and_grants_with_tenant_scope() -> None:
    org = "00000000-0000-0000-0000-0000000000ef"
    run = _row(
        run_id="run-45b", tenant_id=org, organization_id=org,
        workflow_definition_id="workflow", workflow_definition_version="1",
        routing_policy_id="policy", routing_policy_version="1",
        idempotency_key="idem", requested_by="user", status="running",
        started_at=datetime(2026, 8, 4, 12),
    )
    task = _row(
        run_id="run-45b", task_id="task-45b", task_key="execution-45b",
        task_type="execution_plan", objective="objective", idempotency_key="task-idem",
        dependency_ids=[], assigned_worker_type=None, input={"project_id": "project"},
        status="running", started_at=datetime(2026, 8, 4, 12),
    )
    attempt = _row(
        run_id="run-45b", task_id="task-45b", attempt_id="attempt-45b", ordinal=1,
        idempotency_key="attempt-idem", executor_id=None, status="running",
        started_at=datetime(2026, 8, 4, 12), journaled_result={"final_output": {"ok": True}},
    )
    grant = _row(
        grant_id="grant-45b", run_id="run-45b", task_id="task-45b",
        capability_scope="scope", tool_name="search", tool_versions=["1"],
        sensitivity="read_only", max_input_trust="external_untrusted", requires_approval=False,
        issued_at=datetime(2026, 8, 4, 12),
        expires_at=datetime(2026, 8, 4, 13),
    )
    repo = Mock(session=Mock(), get_tasks_for_run=AsyncMock(return_value=[task]),
    ...
    with patch("src.api.services.direct_execution_service.CapabilityRepository", return_value=grants):
    ...
    assert DirectExecutionService._durable_agent_task_context(status) == {
        "run_id": "run-45b", "task_id": "task-45b", "attempt_id": "attempt-45b",
        "organization_id": org,
...
async def test_rehydrate_does_not_cross_tenant_scope() -> None:
    run = _row(
        run_id="run-other", tenant_id="tenant-other",
