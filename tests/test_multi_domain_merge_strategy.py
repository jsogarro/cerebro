...
def service_with_concat_strategy():
    ...
    with patch(
        ...
        bridge.execute.side_effect = execute_side_effect
...
async def test_partial_domain_failure_with_llm_synthesis(service_with_llm_strategy):
    ...
    service_with_llm_strategy.supervisor_bridge.execute.side_effect = (
    ...
    with patch("src.agents.synthesis_agent.SynthesisAgent") as mock_agent_class:
        ...
        status = await service_with_llm_strategy.execute_direct(project)
...
async def test_single_domain_path_unchanged():
    ...
    with patch(
        ...
        bridge.execute.return_value = Mock(
            status="success",
            output={"result": "Single domain result"},
            quality_score=0.9,
        )
