...
class MASRExperimentalRouter(MASRouter):
    ...
    async def get_experiment_results(self, experiment_id: str) -> dict[str, Any]:
        ...
        for variant_id in config.variants:
            variant_data = [
                r for r in self.results_buffer if r.variant_id == variant_id
            ]

            if variant_data:
                variant_results[variant_id] = {
                    "sample_size": len(variant_data),
                    "avg_latency_ms": sum(r.execution_time_ms for r in variant_data)
                    / len(variant_data),
                    "avg_cost": sum(r.cost for r in variant_data) / len(variant_data),
                    "avg_quality": sum(r.quality_score for r in variant_data)
                    / len(variant_data),
                    "error_rate": sum(1 for r in variant_data if r.error_occurred)
                    / len(variant_data),
                    "fallback_rate": sum(1 for r in variant_data if r.fallback_used)
                    / len(variant_data),
                }
        ...
        for exp_id, results in all_results.items():
            ...
            for variant_id, metrics in results["variant_analysis"].items():
                ...
                if "quality" in optimization_goals:
                    score += (
                        metrics.get("avg_quality", 0) * optimization_goals["quality"]
                    )
