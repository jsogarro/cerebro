...
class AdaptiveAllocationEngine:
    ...
    async def register_experiment(
        ...
        if not all(
            variant in allocation_config.initial_allocation for variant in variants
        ):
    ...
    async def _calculate_optimal_allocation(
        ...
        if config.strategy == AllocationStrategy.ADAPTIVE_BANDIT:
            ...
            weighted_values = [
                val * conf for val, conf in zip(arm_values, confidences, strict=True)
            ]
            ...
            for i, variant in enumerate(variants):
                if i < len(bandit.arm_values):
                    performance["variants"][variant] = {
                        ...
                        "sample_count": bandit.arm_counts[i]
