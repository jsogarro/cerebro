...
class MultiVariateAnalyzer:
    ...
    async def analyze_system(
        ...
        interaction_effects = await self._analyze_interactions(pareto_frontier)
    ...
    async def _find_pareto_frontier(
        self,
        evaluation_function: Callable[..., Any],
        parameter_bounds: dict[str, tuple[float, float]],
        n_iterations: int,
        ...
        for metric_name in self.metrics:
            ...
            if metric_range > 0:
                for i in range(1, n - 1):
                    distance = front[i + 1].metrics.get(metric_name, 0) - front[
                        i - 1
                    ...
                    for metric2 in component_metrics[comp2]:
                        ...
                        if len(values1) > 1 and len(values2) > 1:
                            ...
                            if abs(correlation) > 0.7:
                                effect_type = (
                                    "synergistic" if correlation > 0 else "antagonistic"
                                )
                            else:
                                effect_type = "neutral"
    ...
    def _compute_correlations(self, solutions: list[ParetoSolution]) -> pd.DataFrame:
