    ...
    async def analyze_bayesian_ab_test(
        ...
        try:
            ...
            ci_lower, ci_upper = np.percentile(
                posterior_diff,
                [
                    (1 - self.credible_interval_level) / 2 * 100,
                    (1 + self.credible_interval_level) / 2 * 100,
                ],
            )
...
class MultiBanditOptimizer:
    ...
    async def _epsilon_greedy_selection(self) -> BanditResult:
        ...
        expected_reward = (
            self.arm_values[selected_arm] if self.arm_values[selected_arm] > 0 else 0.5
        )
    ...
    async def _ucb_selection(self) -> BanditResult:
        ...
        expected_reward = (
            self.arm_values[selected_arm] if self.arm_counts[selected_arm] > 0 else 0.5
        )
...
class EnhancedStatisticalEngine:
    ...
    async def comprehensive_analysis(
        ...
        for treatment_name in variant_names[1:]:
            ...
            if method in [
                StatisticalMethod.FREQUENTIST_TTEST,
                StatisticalMethod.FREQUENTIST_PROPORTION,
            ]:
                result = await self.frequentist.analyze_ab_test(
                    control_data,
                    treatment_data,
                    "continuous"
                    if method == StatisticalMethod.FREQUENTIST_TTEST
                    else "proportion",
                )
    ...
    async def early_stopping_analysis(
        ...
        try:
            if self.bayesian:
                ...
                can_stop = posterior_prob > prob_threshold or posterior_prob < (
                    1 - prob_threshold
                )

                if can_stop:
                    winner = (
                        variant_names[1] if posterior_prob > 0.5 else variant_names[0]
                    )
            ...
            if significant_results:
                ...
                # Determine winner
                winner = comparison_name.split("_vs_")[
                    0 if result.effect_size > 0 else 1
                ]
