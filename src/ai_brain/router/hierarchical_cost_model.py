...
class SupervisorCostCalculator:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        if self.config:
            for field in [
                "supervisor_instantiation_cost",
                "worker_instantiation_cost",
                "coordination_overhead_cost",
                "message_cost_per_token",
            ]:
    ...
    def calculate_supervisor_runtime_cost(
        ...
        total_runtime_cost = (
            base_runtime_cost * efficiency_multiplier * refinement_multiplier
        )
...
class WorkerCoordinationCostCalculator:
    ...
    def calculate_coordination_overhead(
        ...
        coordination_scaling = (
            1.0 + (worker_count - 1) * self.cost_factors.worker_coordination_scaling
        )
...
class TalkHierCommunicationCostCalculator:
    ...
    def calculate_communication_costs(
        ...
        if enable_consensus_building:
            ...
            consensus_cost = (
                refinement_rounds * self.cost_factors.consensus_building_cost
            )
        ...
        if worker_count > 1:
            ...
            validation_cost = (
                validation_interactions * 0.0002
            )  # Small cost per interaction
...
class HierarchicalCostOptimizer:
    ...
    async def calculate_hierarchical_costs(
        ...
        worker_runtime = (
            worker_count
            * self.cost_factors.worker_runtime_cost_per_sec
            * estimated_runtime
        )
        ...
        communication_costs = (
            self.communication_calculator.calculate_communication_costs(
                tokens_per_round,
                refinement_rounds,
                worker_count,
        ...
        if enable_optimizations:
            ...
            efficiency_savings = (
                self.efficiency_calculator.calculate_efficiency_savings(
                    estimate.gross_cost,
                    enable_resource_pooling=True,
                    enable_caching=routing_strategy != RoutingStrategy.SPEED_FIRST,
                    enable_parallel_optimization=supervision_mode
                    in [SupervisionMode.PARALLEL, SupervisionMode.HYBRID],
    ...
    def _estimate_runtime_seconds(
        ...
        refinement_time = (
            refinement_rounds - 1
        ) * 60.0  # 1 minute per additional round
