...
class RoutingStrategyAnalyzer:
    ...
    def record_strategy_feedback(
        ...
        metrics.average_cost_accuracy = (
            metrics.average_cost_accuracy * (1 - alpha) + feedback.cost_accuracy * alpha
        )
    ...
    def analyze_strategy_performance(
        ...
        for strategy_name, events in self.strategy_events.items():
            ...
            latency_ratios = [
                e.actual_latency_ms / max(e.predicted_latency_ms, 1)
                for e in recent_events
            ]

            analysis_results[strategy_name] = {
                "sample_size": len(recent_events),
                "success_rate": sum(e.execution_success for e in recent_events)
                / len(recent_events),
                "cost_accuracy": {
                    "mean": statistics.mean(cost_accuracies),
                    "std": statistics.stdev(cost_accuracies)
                ...
                "latency_accuracy": {
                    ...
                    "prediction_accuracy": sum(
                        0.8 <= ratio <= 1.2 for ratio in latency_ratios
                    )
                    / len(latency_ratios),
                },
...
class SupervisionFeedbackLearner:
    ...
    def record_execution_feedback(
        ...
        feedback_event = FeedbackEvent(
            ...
            overall_satisfaction=user_feedback.get("satisfaction", 0.8)
            if user_feedback
            else 0.8,
        ...
        self.learning_stats["total_feedback_events"] = (
        ...
        routing_strategy = (
            ...
            if isinstance(routing_strategy_value, str)
        ...
        supervisor_metrics = self.supervisor_performance[
            execution_result.supervisor_type
        ]
        current_total = (
            supervisor_metrics.total_executions
            if supervisor_metrics.total_executions is not None
            else 0
        )
        supervisor_metrics.total_executions = current_total + 1
        if execution_result.status.value == "completed":
            current_success = (
                supervisor_metrics.successful_executions
                if supervisor_metrics.successful_executions is not None
                else 0
            )
    ...
    async def perform_learning_cycle(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.learning_stats["learning_cycles_completed"] = (
                (int(cycles_count) + 1) if isinstance(cycles_count, int) else 1
            )
            ...
            self.learning_stats["routing_improvements"] = (
                int(routing_impr) + improvements["routing"]
                if isinstance(routing_impr, int)
                else improvements["routing"]
            )
            cost_impr = self.learning_stats.get("cost_improvements", 0)
            self.learning_stats["cost_improvements"] = (
                int(cost_impr) + improvements["cost"]
                if isinstance(cost_impr, int)
                else improvements["cost"]
            )
    ...
    def _generate_supervisor_performance_report(self) -> dict[str, dict[str, Any]]:
        ...
        for supervisor_type, metrics in self.supervisor_performance.items():
            ...
            recent_events = [
                ...
                if (
                    event.supervisor_type == supervisor_type
            ...
            report[supervisor_type] = {
                ...
                "recent_performance": {
                    ...
                    "average_quality": statistics.mean(recent_quality)
                    if recent_quality
                    else 0.0,
                    "average_cost_accuracy": statistics.mean(recent_cost_accuracy)
                    if recent_cost_accuracy
                    else 0.0,
                },
                "trends": {
    ...
    def _rank_supervisors_for_context(
        ...
        for supervisor_type, metrics in self.supervisor_performance.items():
            ...
            supervisor_events = [
                e for e in self.feedback_events if e.supervisor_type == supervisor_type
            ]
    ...
    async def save_learning_state(self, file_path: str) -> None:
        ...
        state = {
            ...
            "strategy_metrics": {
                k: asdict(v) for k, v in self.strategy_analyzer.strategy_metrics.items()
            },
            "cost_factors": self.cost_optimizer.cost_factors,
            "cost_predictions": self.cost_optimizer.cost_predictions,
            "supervisor_performance": {
                k: asdict(v) for k, v in self.supervisor_performance.items()
            },
            "learning_stats": self.learning_stats,
