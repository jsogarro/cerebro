...
class ReportStorageService:
    ...
    async def delete_report(self, report_id: UUID, delete_files: bool = True) -> bool:
        ...
        try:
            ...
            if (
                delete_files
                and report.storage_path
                and os.path.exists(report.storage_path)
            ):
    ...
    async def cleanup_old_reports(
        ...
        (
            format_deleted_count,
            format_deleted_ids,
        ) = await self.format_repo.cleanup_orphaned_formats(dry_run)
    ...
    async def verify_report_integrity(self, report_id: UUID) -> dict[str, Any]:
        ...
        if (
            integrity_results["missing_files"]
            or integrity_results["formats_invalid"]
            or integrity_results["errors"]
        ):
