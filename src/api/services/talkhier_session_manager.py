...
class TalkHierSessionManager:
    ...
    def __init__(self) -> None:
        ...
        self.protocol_stats: dict[ProtocolType, dict[str, Any]] = defaultdict(
            lambda: {
                "total_sessions": 0,
    ...
    async def get_analytics(
        ...
        *,
        ...
        all_rounds = [
            m.rounds_completed for m in filtered_sessions if m.rounds_completed > 0
        ]
        average_rounds = sum(all_rounds) / len(all_rounds) if all_rounds else 0

        all_quality = [
            m.final_quality for m in filtered_sessions if m.final_quality > 0
        ]
        average_quality = sum(all_quality) / len(all_quality) if all_quality else 0

        all_consensus = [
            m.final_consensus for m in filtered_sessions if m.final_consensus > 0
        ]
        average_consensus = (
            sum(all_consensus) / len(all_consensus) if all_consensus else 0
        )
