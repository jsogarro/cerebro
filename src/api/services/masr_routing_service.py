...
class MASRRoutingService:
    ...
    async def get_routing_decision(
        ...
        try:
            ...
            complexity = (
                decision.complexity_analysis.level
                if hasattr(decision.complexity_analysis, "level")
                else None
            )
            ...
            domains = (
                decision.complexity_analysis.domains
                if hasattr(decision.complexity_analysis, "domains")
                else []
            )
    ...
    async def estimate_cost(
        ...
        # Extract complexity
        complexity_value = (
            decision.complexity_analysis.level.value
            if hasattr(decision.complexity_analysis, "level")
            else "moderate"
        )
        model_tier = (
        ...
        worker_count = (
            decision.agent_allocation.worker_count
            if hasattr(decision.agent_allocation, "worker_count")
            else 3
        )
    ...
    async def evaluate_strategies(
        ...
        for strategy in strategies:
            ...
            decision = await self.router.route(request.query, strategy=strategy)
        ...
        response = StrategyEvaluationResponse(
            ...
            reasoning=self._generate_strategy_reasoning(best_strategy, comparisons),
    ...
    async def analyze_complexity(
        ...
        if request.include_features:
            ...
            uncertainty = (
                ...
                if hasattr(analysis, "uncertainty_level")
                else 0.5
            )
    ...
    def _get_model_info(self, tier: ModelTier) -> list[ModelInfo]:
        ...
        return [ModelInfo(**config) for config in configs if config["tier"] == tier]

    def _calculate_confidence(self, decision: RoutingDecision) -> float:
        """Calculate routing confidence score"""
        return decision.confidence_score

    def _generate_reasoning(self, decision: RoutingDecision) -> str:
    ...
    def _update_performance_from_feedback(
        ...
        metrics["average_cost"] = (
            alpha * feedback.actual_cost + (1 - alpha) * metrics["average_cost"]
        )
        ...
        metrics["average_quality"] = (
            alpha * feedback.quality_score + (1 - alpha) * metrics["average_quality"]
        )
    ...
    def _generate_cost_recommendations(self, decision: RoutingDecision) -> list[str]:
    ...
    def _check_model_availability(self) -> dict[str, bool]:
        ...
        return {"deepseek": True, "llama": True, "gemini": True}
