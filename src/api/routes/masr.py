...
class StrategyEvaluationRequest(BaseModel):
    ...
    compare_all: bool = Field(False, description="Compare all available strategies")
...
class ComplexityAnalysisRequest(BaseModel):
    ...
    detailed: bool = Field(False, description="Include detailed analysis")
...
async def get_routing_decision(
    ...
    constraints = (
        {"max_cost_usd": request.max_cost_usd} if request.max_cost_usd else None
    )
    ...
    allocated_agents = [
        {
            ...
            "estimated_tokens": decision.complexity_analysis.estimated_tokens
            // max(decision.agent_allocation.worker_count, 1),
        }
    ...
    return RoutingResponse(
        ...
        query_domain=decision.complexity_analysis.domains[0]
        if decision.complexity_analysis.domains
        else QueryDomain.GENERAL,
        ...
        model_recommendations=dict.fromkeys(
            decision.agent_allocation.worker_types,
            decision.optimization_result.primary_model.name,
        ),
...
async def estimate_cost(
    ...
    decision = await masr.route(query=request.query, strategy=request.strategy)
...
async def evaluate_strategies(
    ...
    domain = (
        base_decision.complexity_analysis.domains[0]
        if base_decision.complexity_analysis.domains
        else QueryDomain.GENERAL
    )
    ...
    for strategy in RoutingStrategy:
        ...
        strategy_scores[strategy.value] = {
            ...
            "overall_score": (
                quality_score * 0.4 + latency_score * 0.3 + (1 - cost / 10) * 0.3
            ),
        }

    # Select best strategy
    best_strategy = max(strategy_scores.items(), key=lambda x: x[1]["overall_score"])[0]
...
async def analyze_complexity(
    ...
    domain = (
        decision.complexity_analysis.domains[0]
        if decision.complexity_analysis.domains
        else QueryDomain.GENERAL
    )
    ...
    features = {
        ...
        "requires_research": "research" in request.query.lower()
        or "find" in request.query.lower(),
        "requires_analysis": "analyze" in request.query.lower()
        or "compare" in request.query.lower(),
        "multi_step": complexity in [QueryComplexity.MODERATE, QueryComplexity.COMPLEX],
        "estimated_subtasks": 1
        if complexity == QueryComplexity.SIMPLE
        else (3 if complexity == QueryComplexity.MODERATE else 5),
    }
    ...
    confidence = (
        0.85
        if complexity == QueryComplexity.SIMPLE
        else (0.75 if complexity == QueryComplexity.MODERATE else 0.65)
    )
...
async def get_router_status(
    ...
    cache_stats = {"hits": 823, "misses": 424, "hit_rate": 0.66, "size": 1247}
