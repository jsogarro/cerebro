...
class MultiSupervisorState:
    ...
    active_supervisors: dict[str, str] = field(
        default_factory=dict
    )  # type -> execution_id

    # Execution tracking
    supervisor_results: dict[str, SupervisorExecutionResult] = field(
        default_factory=dict
    )
...
class MultiSupervisorOrchestrator:
    ...
    def __init__(
        ...
        self.max_concurrent_supervisors = self.config.get(
            "max_concurrent_supervisors", 3
        )
    ...
    async def orchestrate_multi_supervisor_query(
        ...
        try:
            ...
            if (
                self.enable_cross_domain_synthesis
                and coordination_result["supervisor_results"]
            ):
                synthesis_result = (
                    await self.cross_domain_synthesizer.synthesize_supervisor_results(
    ...
    async def _plan_supervisor_allocation(
        ...
        return SupervisorAllocation(
            ...
            max_concurrent_supervisors=min(
                len(domains), self.max_concurrent_supervisors
            ),
    ...
    async def _execute_coordinated_supervision(
        ...
        try:
            ...
            if supervisor_results:
                ...
                multi_state.overall_quality_score = sum(quality_scores) / len(
                    quality_scores
                )
    ...
    async def _execute_parallel_supervision(
        ...
        assert allocation is not None
        all_supervisors = [
            allocation.primary_supervisor,
            *allocation.supporting_supervisors,
        ]
        ...
        for supervisor_type in all_supervisors:
            ...
            sub_query = decomposition["domain_subqueries"].get(
                supervisor_type, multi_state.original_query
            )
        ...
        # Process results
        supervisor_results = {}
        for _i, (supervisor_type, result) in enumerate(
            zip(supervisor_tasks.keys(), results, strict=True)
        ):
    ...
    async def _execute_sequential_supervision(
        self, multi_state: MultiSupervisorState, decomposition: dict[str, Any]
    ) -> dict[str, SupervisorExecutionResult]:
        ...
        for supervisor_type in all_supervisors:
            ...
            if len(accumulated_context) > 0:
                await self.inter_supervisor_communicator.coordinate_supervisor_handoff(
                    list(accumulated_context.keys())[-1],
                    supervisor_type,
                    accumulated_context,
                )
    ...
    async def _execute_hierarchical_supervision(
        ...
        if supporting_supervisors:
            ...
            for _i, (supervisor_type, result) in enumerate(
                zip(supporting_tasks.keys(), results, strict=True)
            ):
        ...
        primary_sub_query = decomposition["domain_subqueries"].get(
            primary_supervisor, multi_state.original_query
        )
        ...
        if isinstance(supporting_results, dict):
            primary_task.input_data["supporting_results"] = {
                supervisor_type: result.agent_result.output
                if result.agent_result
                else {}
                for supervisor_type, result in supporting_results.items()
            }

        primary_result = await self._execute_single_supervisor(
            primary_supervisor, primary_task
        )
    ...
    async def _execute_single_supervisor(
        ...
        if (
            hasattr(self.base_orchestrator, "_supervisor_bridge")
            and self.base_orchestrator._supervisor_bridge
        ):
            ...
            if hasattr(self.base_orchestrator, "_supervisor_factory"):
                ...
                if (
                    factory is not None
                    and hasattr(factory, "supervisor_registry")
                    and supervisor_type in factory.supervisor_registry
                ):
                    supervisor_registry = {
                        supervisor_type: factory.supervisor_registry[
                            supervisor_type
                        ].supervisor_class
                    }
        ...
        return SupervisorExecutionResult(
            ...
            agent_result=AgentResult(
                ...
                output={
                    ...
                    "findings": [
                        f"Key finding 1 for {supervisor_type}",
                        f"Key finding 2 for {supervisor_type}",
                    ],
    ...
    async def get_multi_supervisor_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cross_domain_synthesizer": {
                "synthesis_strategies": list(
                    self.cross_domain_synthesizer.synthesis_strategies.keys()
                ),
            },
            "query_decomposer": {
    ...
    async def health_check(self) -> HealthCheckDict:
        ...
        return {
            ...
            "metrics": {
                ...
                "coordination_modes": [
                    mode.value for mode in SupervisorCoordinationMode
                ],
            },
        }
