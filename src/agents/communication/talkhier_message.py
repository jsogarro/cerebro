...
class ProvenanceType(Enum):
    """Message provenance types for security tracking.

    Tracks the source of message content to distinguish trusted vs untrusted
    data across inter-agent trust boundaries. Used by security hardening to
    apply appropriate sanitization and delimiting at prompt interpolation points.
    """

    USER_INPUT = "user_input"  # Direct user input
    TOOL_OUTPUT = "tool_output"  # MCP tool or internal tool output
    LLM_GENERATED = "llm_generated"  # Generated by an LLM agent
    EXTERNAL_WEB = "external_web"  # External web sources (arXiv, PubMed, etc.)
    MEMORY_RETRIEVED = "memory_retrieved"  # Retrieved from long-term memory
...
class TalkHierContent:
    ...
    # Security: Provenance tracking for inter-agent trust boundaries
    source_type: ProvenanceType = ProvenanceType.LLM_GENERATED
